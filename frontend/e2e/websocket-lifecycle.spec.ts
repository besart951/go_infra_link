...
test.describe('websocket lifecycle', () => {
  test('keeps one global stream per concern and disposes project and global streams', async ({
    ...
    const sockets = await observeWebSockets(page);
    ...
    await expectNoConcurrentSockets(sockets, '/api/v1/facility/reference-data/stream');
    await expectNoConcurrentSockets(sockets, '/api/v1/account/notifications/stream');

    const projectID = await createProject(page);
    await expectActiveSocketCount(sockets, `/api/v1/projects/${projectID}/collaboration`, 1);
    await expectNoConcurrentSockets(sockets, `/api/v1/projects/${projectID}/collaboration`);
