...
export const bacnetAlarmRepository: BacnetAlarmRepository = {
  async getSchema(bacnetObjectId: string, signal?: AbortSignal): Promise<AlarmType | null> {
    try {
      const result = await api<AlarmType | null>(
        `/facility/bacnet-objects/${bacnetObjectId}/alarm-schema`,
        { signal }
      );
      return result;
    } catch {
  ...
  async getValues(bacnetObjectId: string, signal?: AbortSignal): Promise<AlarmValue[]> {
    const response = await api<AlarmValuesResponse>(
      `/facility/bacnet-objects/${bacnetObjectId}/alarm-values`,
      { signal }
    );
