...
func (r *spsControllerRepo) DeleteByIds(ids []uuid.UUID) error {
	...
	now := time.Now().UTC()
	return r.db.Model(&domainFacility.SPSController{}).
		Where("id IN ?", ids).
...
func (r *spsControllerRepo) GetPaginatedList(params domain.PaginationParams) (*domain.PaginatedList[domainFacility.SPSController], error) {
	...
	query := r.db.Model(&domainFacility.SPSController{}).Where("deleted_at IS NULL")
	if strings.TrimSpace(params.Search) != "" {
		pattern := "%" + strings.TrimSpace(params.Search) + "%"
		query = query.Where("device_name ILIKE ? OR ip_address ILIKE ?", pattern, pattern)
	}
