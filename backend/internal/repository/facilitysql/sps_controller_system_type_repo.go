...
func (r *spsControllerSystemTypeRepo) GetPaginatedList(params domain.PaginationParams) (*domain.PaginatedList[domainFacility.SPSControllerSystemType], error) {
	...
	if strings.TrimSpace(params.Search) != "" {
		...
		where += " AND (document_name " + sqlutil.LikeOperator(r.dialect) + " ?)"
		args = append(args, pattern)
	}

	countQ := "SELECT COUNT(*) FROM sps_controller_system_types WHERE " + where
	...
	dataQ := "SELECT id, created_at, updated_at, deleted_at, number, document_name, sps_controller_id, system_type_id FROM sps_controller_system_types WHERE " + where + " ORDER BY created_at DESC LIMIT ? OFFSET ?"
...
// Extra helpers used by the service layer.

func (r *spsControllerSystemTypeRepo) GetBySPSControllerIDs(ids []uuid.UUID) ([]*domainFacility.SPSControllerSystemType, error) {
	if len(ids) == 0 {
		return []*domainFacility.SPSControllerSystemType{}, nil
	}

	q := "SELECT id, created_at, updated_at, deleted_at, number, document_name, sps_controller_id, system_type_id " +
		"FROM sps_controller_system_types WHERE deleted_at IS NULL AND sps_controller_id IN (" + sqlutil.Placeholders(len(ids)) + ")"
	q = sqlutil.Rebind(r.dialect, q)
