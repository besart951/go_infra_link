...
var ErrAssignToProjectTransactionNotConfigured = errors.New(
	"project SPS controller assignment transaction is not configured",
)

// AssignToProjectWorkflow is implemented by a transaction-scoped
// ProjectFacilityLinkService. That compatibility workflow retains descendant
// FieldDevice link materialization while this handler owns the outer
// transaction and commit gate.
...
type projectSPSControllerLinkSnapshot struct {
...
func (h *AssignToProjectHandler) Execute(
	...
	committed, err := apptransaction.RunResult(
		...
		func(
			...
			collaborationCommand = appcollaboration.FacilityHierarchyRefreshRequired{
				...
				EntityIDs: []uuid.UUID{result.link.SPSControllerID},
			}
			if _, err := appcollaboration.EnqueueCommand(txCtx, collaborationCommand); err != nil {
				return committedProjectAssignment{}, fmt.Errorf("enqueue project SPS controller assignment: %w", err)
			}
			return result, nil
		},
	)
	...
	if dispatchErr := h.dispatcher.Dispatch(dispatchCtx, collaborationCommand); dispatchErr != nil {
		outcome.DispatchErrors = append(outcome.DispatchErrors, fmt.Errorf(
			"dispatch project SPS controller assignment for project %s: %w",
			command.ProjectID,
			dispatchErr,
		))
	}
...
func executeAssignToProjectTransaction(
	...
	link, err := workflow.CreateSPSController(
		writeCtx,
		command.ProjectID,
	...
	if link == nil || link.ID == uuid.Nil || link.ProjectID != command.ProjectID ||
		link.SPSControllerID != command.SPSControllerID {
		return committedProjectAssignment{}, errors.New("invalid project SPS controller assignment result")
	}
	change, err := buildProjectSPSControllerCreateChange(link)
	if err != nil {
		return committedProjectAssignment{}, err
	}
	return committedProjectAssignment{
		link:    cloneProjectSPSControllerLink(link),
		change:  change,
		batched: batched,
	}, nil
}

func buildProjectSPSControllerCreateChange(
	link *domainProject.ProjectSPSController,
) (mutation.EntityChange, error) {
	after, err := json.Marshal(projectSPSControllerSnapshot(link))
	if err != nil {
		return mutation.EntityChange{}, fmt.Errorf("marshal project SPS controller create snapshot: %w", err)
	}
...
func projectSPSControllerSnapshot(
	link *domainProject.ProjectSPSController,
) projectSPSControllerLinkSnapshot {
	if link == nil {
		return projectSPSControllerLinkSnapshot{}
	}
	return projectSPSControllerLinkSnapshot{
		ID:              link.ID,
		ProjectID:       link.ProjectID,
		SPSControllerID: link.SPSControllerID,
		CreatedAt:       link.CreatedAt,
		UpdatedAt:       link.UpdatedAt,
	}
}

func cloneProjectSPSControllerLink(
	link *domainProject.ProjectSPSController,
