...
func (h *deleteTransactionHarness) factory(
	...
	if !ok || typed.state == nil {
		return nil, errors.New("unexpected SPS delete transaction unit")
	}
...
func (s *deleteWorkflowStub) DeleteByID(ctx context.Context, id uuid.UUID) error {
	...
	if s.state.controller != nil && s.state.controller.ID == id {
		s.state.controller = nil
		s.state.history = append(s.state.history, "sps_controller:delete")
	}
...
func (s *deleteProjectLinkReaderStub) GetBySPSControllerIDs(
	_ context.Context,
	ids []uuid.UUID,
	...
	s.calledBeforeTransaction = s.harness == nil ||
		(s.harness.runnerCalls == 0 && s.harness.committed.controller != nil)
	return s.links, s.err
}

func TestDeleteCapturesDirectRecipientsAndCommitsRootHistoryBeforeDispatch(t *testing.T) {
	controllerID := spsTestUUID(101)
	cabinetID := spsTestUUID(102)
	projectOne := spsTestUUID(111)
	projectTwo := spsTestUUID(112)
	actorID := spsTestUUID(121)
	operationID := spsTestUUID(131)
	eventOne := spsTestUUID(132)
	eventTwo := spsTestUUID(133)
	...
	harness := &deleteTransactionHarness{committed: deleteTransactionState{
		controller: &domainFacility.SPSController{
			Base:             domain.Base{ID: controllerID, CreatedAt: createdAt, UpdatedAt: createdAt},
	...
	links := &deleteProjectLinkReaderStub{
		...
		links: []*domainProject.ProjectSPSController{
			...
			{ProjectID: projectOne, SPSControllerID: controllerID},
			{ProjectID: spsTestUUID(113), SPSControllerID: spsTestUUID(199)},
		},
	}
	...
	if err != nil {
		t.Fatalf("execute SPS delete: %v", err)
	}
	if !links.calledBeforeTransaction || links.calls != 1 ||
		!reflect.DeepEqual(links.received, []uuid.UUID{controllerID}) {
	...
	if harness.committed.controller != nil ||
		!reflect.DeepEqual(harness.committed.history, []string{"sps_controller:delete"}) {
	...
	if snapshot.ID != controllerID || snapshot.ControlCabinetID != cabinetID ||
		snapshot.GADevice == nil || *snapshot.GADevice != gaDevice || snapshot.DeviceName != "SPS-01" {
	...
	for index, raw := range dispatcher.commands {
		command, ok := raw.(appcollaboration.SPSControllerDeleted)
		if !ok {
			t.Fatalf("command: got %T, want SPSControllerDeleted", raw)
		}
		wantProjectID := []uuid.UUID{projectOne, projectTwo}[index]
		if command.ProjectID != wantProjectID || command.SPSControllerID != controllerID ||
			command.ControlCabinetID != cabinetID || command.OperationID != operationID ||
...
func TestDeleteFailureOrCommitFailureDoesNotDispatchOrEscapeTransaction(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			controllerID := spsTestUUID(141)
			harness := &deleteTransactionHarness{
				committed: deleteTransactionState{controller: &domainFacility.SPSController{
					Base:             domain.Base{ID: controllerID},
					ControlCabinetID: spsTestUUID(142),
				}},
				deleteErr: test.deleteErr,
				commitErr: test.commitErr,
			}
			links := &deleteProjectLinkReaderStub{harness: harness, links: []*domainProject.ProjectSPSController{{
				ProjectID:       spsTestUUID(143),
				SPSControllerID: controllerID,
			}}}
			dispatcher := &updateCommandDispatcherStub{}
			handler := NewDeleteHandler(DeleteDependencies{
			...
			_, err := handler.Execute(context.Background(), DeleteCommand{SPSControllerID: controllerID})
...
func TestDeleteMissingSPSControllerPreservesIdempotentSuccess(t *testing.T) {
	harness := &deleteTransactionHarness{}
	...
	outcome, err := handler.Execute(context.Background(), DeleteCommand{
		SPSControllerID: spsTestUUID(151),
	})
	if err != nil {
...
func TestDeleteRecipientLookupFailureIsBestEffortAfterCommit(t *testing.T) {
	controllerID := spsTestUUID(161)
	scopeErr := errors.New("scope lookup failed")
	harness := &deleteTransactionHarness{committed: deleteTransactionState{
		controller: &domainFacility.SPSController{
			Base:             domain.Base{ID: controllerID},
			ControlCabinetID: spsTestUUID(162),
		},
	}}
	...
	outcome, err := handler.Execute(context.Background(), DeleteCommand{SPSControllerID: controllerID})
	if err != nil {
		t.Fatalf("delete with scope failure: %v", err)
	}
	if harness.committed.controller != nil || !outcome.Existed {
